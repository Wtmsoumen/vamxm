...
export default function ContactSection() {
  ...
  return (
    <section id="contact" className="relative overflow-hidden flex justify-end">
      ...
      <div className="relative w-2/3 grid items-center gap-10 px-6 py-14 lg:grid-cols-[1fr_1.2fr] lg:px-10">
        ...
        <AnimateOnScroll anim="right">
          {sent ? (
            ...
            <form className="grid gap-3 sm:grid-cols-2" onSubmit={(e) => { e.preventDefault(); setSent(true); }}>
              ...
              <input className="rounded-lg border bg-white px-4 py-3 text-xs" placeholder="Message" />
              {/* <textarea className="sm:col-span-2 rounded-lg border bg-white px-4 py-3 text-xs" rows={3} placeholder="Your Message" /> */}
              {/* <button type="submit" className="red-gradient hidden shrink-0 rounded-full px-7 py-4 text-sm font-semibold text-white shadow-md transition hover:brightness-110 md:inline-flex" style={{ background: "#df151b" }}>
                Send Message →
              </button> */}
              <div />
              <a className="text-center w-full red-gradient shrink-0 rounded-xl px-7 py-4 text-sm font-semibold text-white shadow-md transition hover:brightness-110" href="#">Send Message</a>
            </form>
          )}
        </AnimateOnScroll>
      </div>
